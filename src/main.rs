use game_template::{game_config, TemplateGame};
...
fn main() {
    ...
    let config = game_config(&root.join("assets").to_string_lossy())
        .with_achievement_save_path(
            root.join("saves/game_template_achievements.json").to_string_lossy(),
        )
    ...
    #[cfg(feature = "editor")]
    editor_integration::run_game_with_editor(TemplateGame::default(), config).unwrap();
    #[cfg(not(feature = "editor"))]
    engine_core::prelude::run_game(TemplateGame::default(), config).unwrap();
}
